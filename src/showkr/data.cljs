...
(defn fetch-set [id]
  (when-not (get-in @world [:data :sets id])
    (swap! world assoc-in [:data :sets id]
      ^{:state :waiting} {})
...
(defn fetch-comments [set-id idx]
  (let [photo (get-in @world [:data :sets set-id :photo idx])]
    (when (and photo (not (:comments photo)))
      (swap! world assoc-in [:data :sets set-id :photo idx :comments]
        ^{:state :waiting} {})
...
(defn fetch-user-sets [username]
  (let [user (get-in @world [:data :users username])]
    (when (and (fetched? user) (not (:sets user)))
      (swap! world assoc-in [:data :users username :sets]
        ^{:state :waiting} {})
...
(defn fetch-user [username]
  (when-not (get-in @world [:data :users username])
    (swap! world assoc-in [:data :users username]
      ^{:state :waiting} {})
